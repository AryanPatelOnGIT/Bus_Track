...
export default function DriverChat() {
  ...
  const [liveRideSessionId, setLiveRideSessionId] = useState<string | null>(null);
  const rideSessionId = liveRideSessionId || userData?.rideSessionId || userData?.vehicleId || 'demo-session';
  ...
  useEffect(() => {
    if (!userData?.vehicleId) {
      setLiveRideSessionId(null);
      return;
    }

    const activeBusRef = ref(rtdb, `activeBuses/${userData.vehicleId}/rideSessionId`);
    const unsubscribe = onValue(activeBusRef, (snapshot) => {
      const value = snapshot.val();
      setLiveRideSessionId(typeof value === 'string' && value.trim() ? value : null);
    });

    return () => off(activeBusRef, 'value', unsubscribe);
  }, [userData?.vehicleId]);
